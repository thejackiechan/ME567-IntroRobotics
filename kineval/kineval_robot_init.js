...
kineval.initRobot = function initRobot() {
    ...
    t = 0; // inits a global time variable
    dt = 0.05;
    actual_index = 0;

    // for IK, initializing first step
    //current_step = 0; ???
}
...
kineval.initRobotJoints = function initRobotJoints() {
    ...
    //   additionally specify parent and child joints for each link
        ...
        robot.joints[x].visited = false; // added these

        if(typeof robot.links[robot.joints[x].parent].children === "undefined"){
            robot.links[robot.joints[x].parent].children = [];
        }   
        robot.links[robot.joints[x].parent].children.push(robot.joints[x].name); 
        robot.links[robot.joints[x].child].parent = robot.joints[x].name;
    }
}
